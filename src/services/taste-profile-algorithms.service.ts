...
class TasteProfileAlgorithmsService {
  ...
  async calculateUserAffinity(userId1: string, userId2: string): Promise<number> {
    try {
      ...
      const confidenceWeight = Math.min(profile1.profileConfidence, profile2.profileConfidence) / 100;
  ...
  async calculateCoffeeAffinity(userId: string, coffeeId: string): Promise<CoffeeAffinityScore> {
    try {
      const userProfile = await TasteProfile.findOne({ userId: new mongoose.Types.ObjectId(userId) });
      ...
      if (coffee.roastLevel && userProfile.preferredCharacteristics.roastLevels.length > 0) {
        const roastMatch = userProfile.preferredCharacteristics.roastLevels
          .find(r => r.level === coffee.roastLevel);
        if (roastMatch) {
      ...
      if (coffee.origin?.country && userProfile.preferredCharacteristics.origins.length > 0) {
        const originMatch = userProfile.preferredCharacteristics.origins
          .find(o => o.country === coffee.origin.country);
        if (originMatch) {
      ...
      if (coffee.flavorProfile?.flavorNotes && userProfile.preferredFlavorProfiles.length > 0) {
        ...
        if (flavorMatches.length > 0) {
          const flavorScore = flavorMatches.reduce((sum, flavor) => {
            const match = userProfile.preferredFlavorProfiles.find(pf => pf.flavorNote === flavor);
      ...
      if (coffee.processingDetails?.method && userProfile.preferredCharacteristics.processingMethods.length > 0) {
        const processingMatch = userProfile.preferredCharacteristics.processingMethods
          .find(p => p.method === coffee.processingDetails.method);
        if (processingMatch) {
  ...
  async findSimilarUsers(userId: string, limit: number = 10): Promise<UserAffinityScore[]> {
    try {
      const targetProfile = await TasteProfile.findOne({ userId: new mongoose.Types.ObjectId(userId) });
      ...
      for (const profile of otherProfiles) {
        ...
        if (affinity > 0.1) { // Minimum similarity threshold
  ...
  async clusterUsersByTaste(k: number = 5): Promise<ClusterResult[]> {
    try {
      ...
      logger.info(`Successfully clustered ${profiles.length} users into ${results.length} clusters`);
      return results;
   ...
   */
  async refineProfileWithCollaborativeFiltering(userId: string): Promise<ITasteProfileDocument | null> {
    try {
      const userProfile = await TasteProfile.findOne({ userId: new mongoose.Types.ObjectId(userId) });
      ...
      logger.info(`Successfully refined taste profile for user ${userId} using ${similarUsers.length} similar users`);
  ...
  private profileToVector(profile: ITasteProfileDocument): TasteProfileVector {
    ...
    const characteristicVector = [
      ...
      ...profile.preferredCharacteristics.processingMethods.slice(0, 3).map(p => p.averageRating * 20)
    ];
   ...
   * @private
   */
  private assignToClusters(vectors: TasteProfileVector[], centroids: number[][]): TasteProfileVector[][] {
    const clusters: TasteProfileVector[][] = Array(centroids.length).fill(null).map(() => []);
   ...
   * @private
   */
  private async analyzeClusterCharacteristics(userIds: string[]): Promise<ClusterResult['characteristics']> {
    const profiles = await TasteProfile.find({
  ...
  private refineAttributesCollaboratively(
    ...
    for (const userAttr of userAttributes) {
      ...
      for (const similarUser of similarUsers) {
        const similarProfile = similarProfiles.find(p => 
          p.userId.toString() === similarUser.userId
        );
        
        if (similarProfile) {
          const similarAttr = similarProfile.preferredAttributes.find(a => 
            a.attribute === userAttr.attribute
          );
