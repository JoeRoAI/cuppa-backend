...
class TasteProfileAggregationService {
  ...
  async generateTasteProfile(userId: string): Promise<ITasteProfileDocument> {
    try {
      ...
      const preferredCharacteristics = await this.calculateCharacteristicPreferences(aggregationData);
      ...
      const tasteProfile = await TasteProfile.findOneAndUpdate(
        { userId: userObjectId },
        {
          ...
          lastRatingDate: aggregationData.ratings.length > 0 
            ? new Date(Math.max(...aggregationData.ratings.map(r => new Date(r.createdAt).getTime())))
            : undefined,
          profileConfidence,
      ...
      logger.info(`Successfully generated taste profile for user ${userId} with confidence ${profileConfidence}%`);
      return tasteProfile;
   ...
   * @private
   */
  private async aggregateUserRatingData(userId: mongoose.Types.ObjectId): Promise<RatingAggregationData> {
    try {
   ...
   * @private
   */
  private async calculateAttributePreferences(data: RatingAggregationData): Promise<IPreferredAttribute[]> {
    const attributes = [
  ...
  private calculateAttributeStats(
    ...
    // Calculate variance
    const variance = attributeValues.reduce((sum, val) => sum + Math.pow(val - averageRating, 2), 0) / count;
    
    // Calculate preference score (0-100)
    // Higher scores indicate stronger preference for this attribute
    const preferenceScore = Math.min(100, Math.max(0, 
   ...
   * @private
   */
  private async calculateFlavorPreferences(data: RatingAggregationData): Promise<IPreferredFlavorProfile[]> {
    ...
    return flavorPreferences
      ...
      .slice(0, 20); // Keep top 20 flavors
  }
   ...
   * @private
   */
  private async calculateCharacteristicPreferences(data: RatingAggregationData): Promise<IPreferredCharacteristics> {
    // Roast levels
    ...
    const roastLevels = Array.from(roastLevelMap.entries()).map(([level, data]) => ({
      level,
      frequency: data.count,
      averageRating: data.ratings.reduce((sum, r) => sum + r, 0) / data.ratings.length
    })).sort((a, b) => b.averageRating - a.averageRating);

    const origins = Array.from(originMap.entries()).map(([country, data]) => ({
      country,
    ...
    const processingMethods = Array.from(processingMap.entries()).map(([method, data]) => ({
      method,
      frequency: data.count,
      averageRating: data.ratings.reduce((sum, r) => sum + r, 0) / data.ratings.length
    })).sort((a, b) => b.averageRating - a.averageRating);
  ...
  private async calculateRatingPatterns(data: RatingAggregationData): Promise<IRatingPatterns> {
    ...
    const overallRatingDistribution = Array.from(distributionMap.entries()).map(([rating, count]) => ({
      rating,
      count,
    ...
    const ratingVariance = ratings.reduce((sum, r) => sum + Math.pow(r - averageOverallRating, 2), 0) / totalRatings;
    ...
    const mostActiveTimeOfDay = hourCounts.size > 0 
      ? Array.from(hourCounts.entries()).sort((a, b) => b[1] - a[1])[0][0]
      : undefined;
      
    const mostActiveDay = dayCounts.size > 0
  ...
  private calculateRatingTrends(ratings: any[]): IRatingPatterns['ratingTrends'] {
    ...
    if (monthlyRatings.length > 0) {
      trends.push({
        period: 'month',
        averageRating: monthlyRatings.reduce((sum, r) => sum + r.overall, 0) / monthlyRatings.length,
    ...
    if (quarterlyRatings.length > 0) {
      trends.push({
        period: 'quarter',
        averageRating: quarterlyRatings.reduce((sum, r) => sum + r.overall, 0) / quarterlyRatings.length,
  ...
  private calculateProfileConfidence(
    ...
    const avgAttributeConfidence = attributes.length > 0
    ...
    return Math.min(100, Math.round(volumeConfidence + attributeConfidence + consistencyConfidence));
  }
   ...
   * @private
   */
  private async createEmptyProfile(userId: mongoose.Types.ObjectId): Promise<ITasteProfileDocument> {
