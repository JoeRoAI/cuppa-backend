...
export const calculateUserAffinity = async (req: Request, res: Response) => {
  try {
    ...
    const affinityScore = await TasteProfileAlgorithmsService.calculateUserAffinity(userId, targetUserId);
...
export const calculateCoffeeAffinity = async (req: Request, res: Response) => {
  try {
    ...
    const coffeeAffinity = await TasteProfileAlgorithmsService.calculateCoffeeAffinity(userId, coffeeId);
    ...
    res.status(StatusCodes.OK).json({
      ...
      data: {
        ...
        recommendation: coffeeAffinity.affinityScore > 0.7 ? 'highly recommended' : 
                      coffeeAffinity.affinityScore > 0.4 ? 'recommended' : 'might not be your taste'
      },
    });
  } catch (error: any) {
    ...
    res.status(StatusCodes.OK).json({
      ...
      data: {
        ...
        similarUsers: similarUsers.map(user => ({
          ...
          compatibility: user.affinityScore > 0.7 ? 'high' : user.affinityScore > 0.4 ? 'medium' : 'low'
        })),
...
export const refineProfileWithCollaborativeFiltering = async (req: Request, res: Response) => {
  try {
    ...
    const refinedProfile = await TasteProfileAlgorithmsService.refineProfileWithCollaborativeFiltering(userId);
...
export const triggerUpdate = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const TasteProfileUpdateService = (await import('../services/taste-profile-update.service')).default;
