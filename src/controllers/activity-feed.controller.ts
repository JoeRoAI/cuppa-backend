...
class ActivityFeedController {
  ...
  async createActivity(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const { activityType, targetId, targetType, targetUserId, content, metadata, visibility } =
        req.body;
      ...
      // Create activity
      const activity = await ActivityFeedService.recordActivity(userId, activityType, {
        targetId,
        targetType,
        targetUserId,
        content,
